...
#Region "Atributos"
    ...
    Dim _overlay2 As New GMapOverlay
    Dim _listRepAct As New List(Of DataRow)
    Dim _listVenAct As New List(Of DataRow)
    Dim _RutaImagenes As String
    Dim _Induracion As Byte = 5
...
#Region "Metodos Privados"
    ...
    Private Sub _PIniciarTodo()
        ...
        TimerRuta.Interval = 10000
        _prCargarGridPersonal()
        _prCargarGridPersonal2()
    ...
    Private Sub _PCargarcombopersonas()
        ...
        With CbPersonal.DropDownList
            ...
            .Columns.Add(Dt.Columns("cbdesc").ToString).Width = 150
            .Columns(1).Caption = "Nombre"
        End With

        CbPersonal.ValueMember = Dt.Columns("cbnumi").ToString
        CbPersonal.DisplayMember = Dt.Columns("cbdesc").ToString
    ...
    Private Sub _prCargarGridPersonal2()
        ...
        Gr_vendedores.DataSource = dt
        Gr_vendedores.RetrieveStructure()

        'dar formato a las columnas
        With Gr_vendedores.RootTable.Columns("cbnumi")
            .Caption = "Cod".ToUpper
            .Width = 50
            .HeaderAlignment = Janus.Windows.GridEX.TextAlignment.Center
            .CellStyle.TextAlignment = Janus.Windows.GridEX.TextAlignment.Far
        ...
        With Gr_vendedores.RootTable.Columns("cbdesc")
            .Caption = "Nombre"
            .Width = 250
            .HeaderAlignment = Janus.Windows.GridEX.TextAlignment.Center
        End With

        With Gr_vendedores.RootTable.Columns("estado")
            .Caption = "Check"
            .Width = 50
        End With

        With Gr_vendedores.RootTable.Columns("cbfot")
            .Visible = False
        End With

        With Gr_vendedores.RootTable.Columns("color")
            .Visible = False
        End With
        ...
        For Each fila As DataRow In dt.Rows
            ''  Dim _color As Integer = _prGetColorByPos(i)
            Dim _color As Integer = 7
            fila.Item("estado") = 0
            fila.Item("color") = _color

            'Dim _estiloFila As New GridEXFormatStyle()
            'Dim _fil As Janus.Windows.GridEX.GridEXRow
            '_fil = Gr_Vendedores.GetRow(i)
            '_estiloFila.BackColor = _prGetoColor(color)
            '_fil.RowStyle = _estiloFila

            Dim fc As GridEXFormatCondition
            fc = New GridEXFormatCondition(Gr_vendedores.RootTable.Columns("color"), ConditionOperator.Equal, _color)
            'fc.FormatStyle.BackColor = _listColores(_color)
            'fc.FormatStyle.BackColorGradient = _listColores(_color)
            Gr_vendedores.RootTable.FormatConditions.Add(fc)
            i = i + 1
        Next
        ...
        End With
    End Sub
    Private Sub _CargarRecorrido(ByRef NumiVend As Integer)

        Dim dt As New DataTable
        dt = L_Recorrido(NumiVend, DtFecha.Value.ToShortDateString)
        If dt.Rows.Count = 0 Then
            ToastNotification.Show(Me, "Persona no tiene Recorrido.".ToUpper,
            ...
            Exit Sub
        End If
        GrRecorrido.DataSource = dt
        GrRecorrido.RetrieveStructure()

        'dar formato a las columnas
        With GrRecorrido.RootTable.Columns("ldchof")
            .Caption = "Cod".ToUpper
            .Width = 50
            .HeaderAlignment = Janus.Windows.GridEX.TextAlignment.Center
            .CellStyle.TextAlignment = Janus.Windows.GridEX.TextAlignment.Far
        ...
        With GrRecorrido.RootTable.Columns("ldfec")
            .Caption = "Fecha"
            .Width = 120
            .HeaderAlignment = Janus.Windows.GridEX.TextAlignment.Center
        End With

        With GrRecorrido.RootTable.Columns("ldhora")
            .Caption = "Hora"
            .Width = 80
        End With
        With GrRecorrido.RootTable.Columns("ldnumi")
            .Caption = "Hora"
            .Visible = False
            .Width = 80
        End With
        With GrRecorrido.RootTable.Columns("lblat")
            .Caption = "Hora"
            .Visible = False
            .Width = 80
        End With
        With GrRecorrido.RootTable.Columns("lblongi")
            .Caption = "Hora"
            .Visible = False
            .Width = 80
        End With
    ...
    Private Sub _prDibujarPuntosEnMapa()
        ...
        For Each elem As DataRow In _listRepAct.ToArray
            ...
            End If
        Next
        If GrRecorrido.RowCount > 0 Then
            If SwitchButton1.Value = True Then
                color = 20
                nombre = GrRecorrido.GetValue("ldhora")
                Dim Lat As String = GrRecorrido.GetValue("lblat")
                Dim Longi As String = GrRecorrido.GetValue("lblongi")
                _PDibujarPunto(_overlay, New PointLatLng(Lat, Longi), color, "  ", nombre)
            End If
        End If
...
#End Region
    ...
    Private Sub tbTracking_ValueChanged(sender As Object, e As EventArgs) Handles tbTracking.ValueChanged
        If tbTracking.Value = True Then
            ...
            Gr_vendedores.AllowEdit = InheritableBoolean.True
            Sw_Repartidor.Enabled = True
            Sw_Vendedor.Enabled = True
            SwitchButton1.Enabled = True
        Else
            TimerRuta.Stop()
            grRepartidores.AllowEdit = InheritableBoolean.False
            Gr_vendedores.AllowEdit = InheritableBoolean.False
            Sw_Repartidor.Enabled = False
            Sw_Repartidor.Value = False
            Sw_Vendedor.Value = False
            Sw_Vendedor.Enabled = False
            ...
            For Each fil As DataRow In CType(grRepartidores.DataSource, DataTable).Rows
                ...
                End If
            Next
            For Each fil As DataRow In CType(Gr_vendedores.DataSource, DataTable).Rows
                If (fil.Item("estado")) Then
                    fil.Item("estado") = False
                End If
            Next
    ...
    Private Sub Timer1_Tick(sender As Object, e As EventArgs) Handles Timer1.Tick
        ...
        'Timer1.Enabled = False
    End Sub
    Private Sub Gr_vendedores_EditingCell(sender As Object, e As EditingCellEventArgs) Handles Gr_vendedores.EditingCell
        If (tbTracking.Value) Then
            If (e.Column.Key.Equals("estado")) Then
                e.Cancel = False
            Else
    ...
    Private Sub Gr_vendedores_CellValueChanged(sender As Object, e As ColumnActionEventArgs) Handles Gr_vendedores.CellValueChanged
        If Gr_vendedores.Row >= 0 And e.Column.Key = "estado" Then
            If Gr_vendedores.GetValue("estado").ToString = "True" Then
                _listRepAct.Add(CType(Gr_vendedores.DataSource, DataTable).Rows(Gr_vendedores.Row))
                tbTracking.Focus()
            Else
                Dim i As Integer = _listVenAct.IndexOf(CType(Gr_vendedores.DataSource, DataTable).Rows(Gr_vendedores.Row))
                _listRepAct.RemoveAt(i)
                tbTracking.Focus()
            End If
    ...
    Private Sub Sw_Repartidor_ValueChanged(sender As Object, e As EventArgs) Handles Sw_Repartidor.ValueChanged
        Dim I As Integer
        If Sw_Repartidor.Value = True Then
            I = 0
            For Each fil As DataRow In CType(grRepartidores.DataSource, DataTable).Rows
                fil.Item("estado") = True
                _listRepAct.Add(CType(grRepartidores.DataSource, DataTable).Rows(I))
                I = I + 1
            Next
        Else
            I = 0
            For Each fil As DataRow In CType(grRepartidores.DataSource, DataTable).Rows
                fil.Item("estado") = False
                _listRepAct.RemoveAt(0)
                I = I + 1
            Next
        End If
    End Sub

    Private Sub Sw_Vendedor_ValueChanged(sender As Object, e As EventArgs) Handles Sw_Vendedor.ValueChanged
        Dim I As Integer
        If Sw_Vendedor.Value = True Then
            I = 0
            For Each fil As DataRow In CType(Gr_vendedores.DataSource, DataTable).Rows
                fil.Item("estado") = True
                _listRepAct.Add(CType(Gr_vendedores.DataSource, DataTable).Rows(I))
                I = I + 1
            Next
        Else
            I = 0
    ...
    Private Sub SwitchButton1_ValueChanged(sender As Object, e As EventArgs) Handles SwitchButton1.ValueChanged
        If SwitchButton1.Value = True Then
            CbPersonal.Enabled = True
        Else
            CbPersonal.Enabled = False
        End If
    End Sub

    Private Sub GrRecorrido_CellValueChanged(sender As Object, e As ColumnActionEventArgs) Handles GrRecorrido.CellValueChanged
        '    Dim color As Integer = 16
        '    Dim nombre As String = GrRecorrido.GetValue("ldhora")
        '    Dim Lat As String = GrRecorrido.GetValue("lblat")
        'Dim Longi As String = GrRecorrido.GetValue("lblong")
        '_PDibujarPunto(_overlay, New PointLatLng(Lat, Longi), color, "  ", nombre)
    End Sub

    Private Sub GrRecorrido_SelectionChanged(sender As Object, e As EventArgs) Handles GrRecorrido.SelectionChanged
        'MsgBox("Hola")
        Dim color As Integer = 20
        Dim nombre As String = GrRecorrido.GetValue("ldhora")
        Dim Lat As String = GrRecorrido.GetValue("lblat")
        Dim Longi As String = GrRecorrido.GetValue("lblongi")
        _PDibujarPunto(_overlay, New PointLatLng(Lat, Longi), color, "  ", nombre)
