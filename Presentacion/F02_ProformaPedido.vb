...
Public Class F02_ProformaPedido
...
#Region "Metodos Privados"
    Private Sub _PIniciarTodo()
        ...
        Me.Text = "P R O F O R M A   P E D I D O S"
    ...
    Private Sub _PCargarBuscador(tipo As Integer, Optional where As String = "")
        Try
            ...
            If tipo = 1 Then
                JGr_Buscador.DataSource = L_prListaProforma()
            Else
                Dim tPedidos = L_prListaProformaPorFecha(tbFechaDel.Value.Date.ToString("yyyy/MM/dd"), tbFechaAl.Value.Date.ToString("yyyy/MM/dd"))
            ...
            With JGr_Buscador.RootTable.Columns("pmnumi")
            ...
            With JGr_Buscador.RootTable.Columns("pmfdoc")
            ...
            With JGr_Buscador.RootTable.Columns("pmhora")
            ...
            With JGr_Buscador.RootTable.Columns("pmccli")
            ...
            With JGr_Buscador.RootTable.Columns("pmzona")
            ...
            With JGr_Buscador.RootTable.Columns("pmobs")
            ...
            With JGr_Buscador.RootTable.Columns("pmobs2")
                .Visible = False
            End With

            With JGr_Buscador.RootTable.Columns("pmest")
                .Visible = False
            End With
            ...
            With JGr_Buscador.RootTable.Columns("cclongi")
                .Visible = False
            End With
            With JGr_Buscador.RootTable.Columns("pmap")
                .Visible = False
            End With
            With JGr_Buscador.RootTable.Columns("pmpg")
                .Visible = False
            End With
            With JGr_Buscador.RootTable.Columns("ccultvent")
                .Visible = False
            End With
            With JGr_Buscador.RootTable.Columns("pmrepa")
                .Visible = False
            End With
    ...
    Private Sub _PCargarGridDetalle(idCabecera As Integer)
        ...
        dtProd = L_ProformaDetalle(-1, idCabecera)
    ...
    Private Sub _PMostrarRegistro(_N As Integer)
        ...
        With JGr_Buscador '_Dsencabezado.Tables(0).Rows(_N)
            Tb_Id.Text = .GetValue("pmnumi")
            Tb_Fecha.Value = .GetValue("pmfdoc")
            Tb_Hora.Text = .GetValue("pmhora")
            Tb_CliCod.Text = .GetValue("pmccli")
            ...
            Tb_CliCodZona.Text = .GetValue("pmzona")
            Tb_CliCateg.Text = .GetValue("cccat")
            Tb_Zona.Text = .GetValue("cedesc")
            Tb_Observaciones.Text = .GetValue("pmobs")

            If .GetValue("pmest") = 0 Then
            ...
            cbDistribuidor.Value = .GetValue("pmrepa")
            cbPreVendedor.Clear()
    ...
    Private Function _PValidar() As Boolean
        Try
            ...
            For i = 0 To dt.Rows.Count - 1
                If dt.Rows(i).Item("pntotal").ToString = String.Empty Then
                    ToastNotification.Show(Me, "Falta calcular el total en algún producto del detalle, por favor presione el botón aplicar descuentos".ToUpper, My.Resources.WARNING, 5500, eToastGlowColor.Green, eToastPosition.TopCenter)
                    _Error = True
                Else
                    sumTotal = sumTotal + dt.Rows(i).Item("pntotal")
                End If
            Next
    ...
    Private Sub _PGrabarRegistro()
        Try
            ...
            If _Nuevo Then
                ...
                L_ProformaCabecera_Grabar(Tb_Id.Text, Date.Now.Date.ToString("yyyy/MM/dd"), Tb_Hora.Text, Tb_CliCod.Text, Tb_CliCodZona.Text, cbDistribuidor.Value.ToString, Tb_Observaciones.Text, IIf(_nuevoBasePeriodico = True, "10", "1"), "1", "0")
                ...
                For i = 0 To JGr_DetallePedido.RowCount - 1
                    ...
                    L_ProformaDetalle_Grabar(Tb_Id.Text, codProd, cant, precio, subTotal, desc, total, flia, atributo)
                Next
                ...
                Tb_Fecha.Focus()
                ToastNotification.Show(Me, "Codigo de Proforma " + Tb_Id.Text + " Grabado con éxito.", My.Resources.GRABACION_EXITOSA, 5000, eToastGlowColor.Green, eToastPosition.BottomLeft)
                _PLimpiar()
            ...
            Else
                ...
                Dim oaest As String = L_fnObtenerDatoTabla("TP009", "pmest", "pmnumi=" + Tb_Id.Text.Trim)

                L_ProformaCabacera_Modificar(Tb_Id.Text, Tb_Fecha.Value.ToString("yyyy/MM/dd"), Tb_Hora.Text, Tb_CliCod.Text, Tb_CliCodZona.Text, cbDistribuidor.Value.ToString, Tb_Observaciones.Text, IIf(_nuevoBasePeriodico = True, "10", oaest))

                'modificar detalle
                L_ProformaDetalle_Borrar(Tb_Id.Text)
                Dim codProd, cant, precio, subTotal, desc, total, flia, atributo As String
                Dim i As Integer
                For i = 0 To JGr_DetallePedido.RowCount - 1
                    JGr_DetallePedido.Row = i
                ...
                ToastNotification.Show(Me, "Codigo de Proforma " + Tb_Id.Text + " Modificado con éxito.", My.Resources.GRABACION_EXITOSA, 5000, eToastGlowColor.Green, eToastPosition.BottomLeft)
                '_Deshabilitar()
    ...
    Private Sub _PEliminarRegistro()
        ...
        If _Result = MsgBoxResult.Yes Then
            ...
            Dim res As Boolean = L_EliminarProforma(Tb_Id.Text, mensajeError)
            If res Then

                Dim img As Bitmap = New Bitmap(My.Resources.checked, 50, 50)

                ToastNotification.Show(Me, "Código de Proforma ".ToUpper + Tb_Id.Text + " eliminado con éxito.".ToUpper,
                                          img, 2000,
    ...
    Private Sub _PGenerarPedidosPeriodicamente()
        ...
        If grabar = True Then
            ...
            L_PedidoCabecera_Grabar(idPedido, fecha, Now.Hour.ToString + ":" + Now.Minute.ToString, Tb_CliCod.Text, Tb_CliCodZona.Text, cbDistribuidor.Value.ToString, Tb_Observaciones.Text, "1", "1", "1", 0, 1)
...
#End Region
    ...
    Public Function _fnExisteProducto(idprod As Integer) As Boolean
        For i As Integer = 0 To CType(JGr_DetallePedido.DataSource, DataTable).Rows.Count - 1 Step 1
            Dim _idprod As Integer = CType(JGr_DetallePedido.DataSource, DataTable).Rows(i).Item("pncprod")
    ...
    Private Sub Tb_CantProd_KeyDown(sender As Object, e As KeyEventArgs) Handles Tb_CantProd.KeyDown
        If e.KeyData = Keys.Enter Then
            ...
            Modelo.MGlobal.gs_MBanderaEnter = True
                If atributo = -1 Then
                    CType(JGr_DetallePedido.DataSource, DataTable).Rows(pos).Item("pnpcant") = Tb_CantProd.Text
                    CType(JGr_DetallePedido.DataSource, DataTable).Rows(pos).Item("pnptot") = CDbl(1) * precio
                ...
                Else
                    CType(JGr_DetallePedido.DataSource, DataTable).Rows(pos).Item("pnpcant") = Tb_CantProd.Text
                    CType(JGr_DetallePedido.DataSource, DataTable).Rows(pos).Item("pnptot") = CDbl(Tb_CantProd.Text) * precio
    ...
    Private Sub _PGrabarRegistroDirecto()
        ...
        If _Nuevo Then
            ...
            L_PedidoCabecera_Grabar(Tb_Id.Text, Date.Now.Date.ToString("yyyy/MM/dd"), Tb_Hora.Text, Tb_CliCod.Text, Tb_CliCodZona.Text, cbDistribuidor.Value.ToString, Tb_Observaciones.Text, IIf(_nuevoBasePeriodico = True, "10", "2"), "1", "0", 0, 1)
    ...
    Private Sub P_prImprimirFacturar(numi As String, impFactura As Boolean, grabarPDF As Boolean, nit As String)
        ...
        L_Actualiza_Dosificacion(_numidosif, _NumFac, numi)
    End Sub
    Private Sub btnVentaDirecta_Click(sender As Object, e As EventArgs)
    ...
    Private Sub btAplicarDesc_Click(sender As Object, e As EventArgs) Handles btAplicarDesc.Click
        ...
        For i = 0 To JGr_DetallePedido.RowCount - 1
            codpro = CType(JGr_DetallePedido.DataSource, DataTable).Rows(i).Item("pncprod")
            total1 = CType(JGr_DetallePedido.DataSource, DataTable).Rows(i).Item("pnptot")

            dt = L_fnMostrarDescuentosPrecios(codpro)

            'Cálculo de descuentos si es sin familia
            If CType(JGr_DetallePedido.DataSource, DataTable).Rows(i).Item("pnfamilia") = 1 Then
                cant = CType(JGr_DetallePedido.DataSource, DataTable).Rows(i).Item("pnpcant")
                ...
                Else
                    descuento = 0
                End If
                CType(JGr_DetallePedido.DataSource, DataTable).Rows(i).Item("pndesc") = descuento
                CType(JGr_DetallePedido.DataSource, DataTable).Rows(i).Item("pntotal") = total1 - descuento
                descuento = 0
                total2 = 0

            Else
                'Cálculo de descuentos por familia
                Dim familia = CType(JGr_DetallePedido.DataSource, DataTable).Rows(i).Item("pnfamilia")
                Dim cantnormal = CType(JGr_DetallePedido.DataSource, DataTable).Rows(i).Item("pnpcant")
