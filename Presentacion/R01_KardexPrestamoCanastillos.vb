...
Public Class R01_KardexPrestamoCanastillos
...
#Region "Metodos"
    ...
    Private Sub P_prCargarReporte()
        ...
        Else
            ...
            If (Not TbCodigoCliente.Text.Trim = String.Empty) Then
                ...
                dt = L_prKardexPrestamoCanastillos(CInt(TbCodigoCliente.Text.Trim), tbFechaI.Value.ToString("dd/MM/yyyy"))
            Else
        ...
        If (dt.Rows.Count > 0) Then
            Dim saldoA As Integer = L_prObtenerSaldoAnterior(CInt(TbCodigoCliente.Text.Trim), tbFechaI.Value.ToString("dd/MM/yyyy"))
            armarKardex(dt, saldoA)
            Dim objrep As New R_PrestamoCanastillos
            objrep.SetDataSource(dt)
            objrep.SetParameterValue("cliente", TbNombreCliente.Text)
            objrep.SetParameterValue("saldo", saldoA)
    ...
    Private Sub armarKardex(ByRef dt As DataTable, saldo As Integer)

        dt.Rows(0).Item("deuda") = dt.Rows(0).Item("cant1")
        dt.Rows(0).Item("saldo") = dt.Rows(0).Item("deuda") - dt.Rows(0).Item("cant2") + saldo

        For i = 1 To dt.Rows.Count - 1 Step 1
            Dim sal As Integer = dt.Rows(i - 1).Item("saldo")
            dt.Rows(i).Item("deuda") = sal + dt.Rows(i).Item("cant1")
            dt.Rows(i).Item("saldo") = dt.Rows(i).Item("deuda") - dt.Rows(i).Item("cant2")
