...
Public Class R01_VentasUtilidad
...
#Region "METODOS PRIVADOS"
    ...
    Private Sub armarReporte2(ByRef dt As DataTable)
        ...
        For k = 0 To diferenciaMeses Step 1
            ...
            dt.Columns.Add("utilporce" + k.ToString)
            Dim dtaux As DataTable = L_prReporteVentasDetalladaxMes2(mesI + cont, anioI, tbFechaI.Value.ToString("dd/MM/yyyy"), tbFechaF.Value.ToString("dd/MM/yyyy"), cbProv.Value)
            If dtaux.Rows.Count > 0 Then
                For j = 0 To dtaux.Rows.Count - 1 Step 1
                    For i = 0 To dt.Rows.Count - 1 Step 1

                        If dt.Rows(i).Item("ccnumi") = dtaux.Rows(j).Item("ccnumi") Then
                            dt.Rows(i).Item((2 + k * 4)) = dtaux.Rows(j).Item("total")
                            dt.Rows(i).Item((2 + k * 4) + 1) = dtaux.Rows(j).Item("peso")
                            dt.Rows(i).Item((2 + k * 4) + 2) = dtaux.Rows(j).Item("utilidad")
                            dt.Rows(i).Item((2 + k * 4) + 3) = dtaux.Rows(j).Item("utilporce")
                            Exit For
                        End If
    ...
    Private Sub armarReporte(ByRef dt As DataTable)
        ...
        'For i = 0 To dt.Rows.Count - 1 Step 1
        For j = 0 To 3 Step 1
            dt = L_prReporteVentasDetalladaxMes2(mesI + cont, anioI, tbFechaI.Value.ToString("dd/MM/yyyy"), tbFechaF.Value.ToString("dd/MM/yyyy"), cbProv.Value, dt, j)
            'Dim dtaux As DataTable = L_prReporteVentasDetalladaxMes(dt.Rows(i).Item("ccnumi"), mesI + cont, anioI, tbFechaI.Value.ToString("dd/MM/yyyy"), tbFechaF.Value.ToString("dd/MM/yyyy"), cbProv.Value)
            'If dtaux.Rows.Count > 0 Then
    ...
    Private Sub _prCargarReporte()
        ...
        _dt = L_prReporteVentasDetallada(tbFechaI.Value.ToString("yyyy/MM/dd"), tbFechaF.Value.ToString("yyyy/MM/dd"), cbProv.Value, cbTipC.Value)
        armarReporte(_dt)
        If (_dt.Rows.Count > 0) Then
            ...
            objrep.SetParameterValue("titulo", "REPORTE DE VENTAS POR MESES")
...
#End Region
    ...
    Private Sub btnExportar_Click(sender As Object, e As EventArgs) Handles btnExportar.Click
        Dim _dt As New DataTable
        _dt = L_prReporteVentasDetallada2(tbFechaI.Value.ToString("yyyy/MM/dd"), tbFechaF.Value.ToString("yyyy/MM/dd"), cbProv.Value, cbTipC.Value)
        armarReporte2(_dt)
        RellenarVacios(_dt)
    ...
    Public Function P_ExportarExcel(_ruta As String) As Boolean
        ...
        If (1 = 1) Then 'If(_directorio.ShowDialog = Windows.Forms.DialogResult.OK) Then
            ...
            Try
                ...
                Dim _archivo As String = _ubicacion & "\ReporteVentaCliente_" & Now.Date.Day &
